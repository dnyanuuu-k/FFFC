...
public class RNSelectionMenuModule extends ReactContextBaseJavaModule {
  ...
  public void Show(final ReadableMap props, final Callback callback) {
    ...
    if (presentationType < 3) {
      ...
      final ArrayList<SampleModel> items = new ArrayList<>();
      for (int i = 0; i < values.size(); i++) {
        items.add(new SampleModel(values.getString(i)));
      }
      final Integer hintColor = Color.parseColor(String.valueOf(searchTintColor));
      UiThreadUtil.runOnUiThread(new Thread(new Runnable() {
        public void run() {                    
          ...
          dialog.show();          
          dialog.getTitleTextView().setTextColor(Color.BLACK);
          dialog.getSearchBox().setHintTextColor(hintColor);
        }
      }));
    }
  }
