...
class MultipleImagePickerModule(reactContext: ReactApplicationContext) :
    ...
    private fun setCropOptions(libOption: ReadableMap) {
        ...
        if(libOption.hasKey("ratioWidth") && libOption.hasKey("ratioHeight")) {
            options.setMultipleCropAspectRatio(
                AspectRatio("Default", libOption.getDouble("ratioWidth").toFloat(), libOption.getDouble("ratioHeight").toFloat())
            );
        }        
