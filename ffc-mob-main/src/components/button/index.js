...
const getTypeStyle = (type) => {
	switch (type) {
		...
		case Button.ICON_PRIMARY:
			return { btnSty: defaultStyle.main2, txtSty: defaultStyle.txt7 };
		case Button.ICON_SECONDARY:
			return { btnSty: defaultStyle.secondary2, txtSty: defaultStyle.txt3 };
		...
		case Button.OUTLINE_DANGER:
			return { btnSty: defaultStyle.outline3, txtSty: defaultStyle.txt6 };
...
export default class Button extends PureComponent {
	...
	static ICON_SECONDARY = 8;

	static OUTLINE_PRIMARY = 2;
	static OUTLINE_DANGER = 10;
	...
	render() {
		...
		return (
			<TouchableOpacity style={[btnSty, style]} onPress={onPress}>
				{icon ? (
					<FeatherIcon name={icon} color={txtSty.color} size={iconSize} />
				) : null}
				{busy ? (
